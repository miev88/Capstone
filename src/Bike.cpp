...
Bike::Bike(size_t name) : Vehicle(name)
{
    _type = ObjectType::objectBike;
    _speed = 350; // m/s
}
...
void Bike::drive()
{
    ...
    std::cout << "Bike #" << _id << "::drive: thread id = " << std::this_thread::get_id() << std::endl;
    lck.unlock();
    ...
    while (true)
    {
	    ...
	    // make bike drive in its lane at a certain position
	    int delta = 110;
	    ...
	    yv = y1 + completion * dy;
			...
			for (auto street : streetOptions) 
			{
			   if (street->getName() == 3)	{ nextStreet = street; }
 		 	}
		    }
		    else if (_currStreet->getName() == 2)
	            {
			...
			for (auto street : streetOptions) 
			{
			   if (street->getName() == 1)	{ nextStreet = street; }
 		 	}
		    }
		    else 
		    {
