...
Car::Car(size_t name) : Vehicle(name)
{
    _type = ObjectType::objectCar;
    _speed = 400; // m/s
}
...
void Car::drive()
{
    ...
    std::cout << "Car #" << _id << "::drive: thread id = " << std::this_thread::get_id() << std::endl;
    lck.unlock();
    ...
    while (true)
    {
	    ...
	    // make car drive in its lane at a certain position
	    int delta = 75;
